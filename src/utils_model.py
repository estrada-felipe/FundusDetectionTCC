...
def inspect_models(ModelFactory, arch, num_classes, input_shape):
    ...
    try:
        # Resumo da estrutura (funciona apenas para modelos com entradas padrão)
        summary(model, input_size=(CHANNELS, IMAGE_SIZE_H, IMAGE_SIZE_W))
    except Exception as e:
        print(f"⚠️ Não foi possível gerar o resumo do modelo: {e}")


def salvar_modelo_completo(args, model, optimizer, modelo_nome, epoch, history, config):
    # Diretório para o modelo específico
...
def avaliar_modelo_no_teste(model, test_ds, modl, class_names, plot_conf=False):
    ...
    with torch.no_grad():
        for images, labels in test_ds:
            ...
            # Calcular o tempo de inferência e adicionar à lista
...
def adjust_pos_embed(pos_embed_checkpoint, current_pos_embed):
    ...
    if pos_embed_checkpoint.shape != current_pos_embed.shape:
        print(f"⚠️ Ajustando os embeddings posicionais de {pos_embed_checkpoint.shape} para {current_pos_embed.shape}")
...
def carregar_modelo(modelo_path):
    ...
    # Reconstruir o modelo com as configurações salvas
    ...
    # Ajustar os embeddings posicionais, se necessário
